...
Status HiveDataSource::_init_partition_values() {
    ...
    for (const auto& item : _hive_table->partition_map()) {
        ...
        if (item.second != nullptr) {
            ...
            LOG(ERROR) << "LXH: P_MAP_NULL: " << tmp_i << ":" << item.first << ":" << item.second;
        }
        tmp_i++;
    }
...
void HiveDataSource::_init_tuples_and_slots(RuntimeState* state) {
    ...
    LOG(ERROR) << "LXH: PAR_COUNT: " << _hive_table->partition_columns().size();
    for (int i = 0; i < _hive_table->partition_columns().size(); i++) {
        LOG(ERROR) << "LXH: PAR: " << i << ":" << _hive_table->partition_columns()[i].column_name;
    }
