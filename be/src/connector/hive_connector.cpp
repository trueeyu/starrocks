...
Status HiveDataSource::_init_partition_values() {
    ...
    // Build partition_key ExprContexts in the fragment-local pool. They cannot be
    // shared from HdfsPartitionDescriptor (which lives in the query-level pool):
    // ExprContext::prepare captures a fragment RuntimeState, so the contexts must
    // live and close within the fragment scope.
    const auto& thrift_partition_key_exprs = partition_desc->thrift_partition_key_exprs();
    RETURN_IF_ERROR(ExprFactory::create_expr_trees(&_pool, thrift_partition_key_exprs, &_partition_values,
                                                   _runtime_state));
