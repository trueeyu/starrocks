...
public:
    ...
    void update_min_max(CppType val) {
        ...
        if constexpr (is_min) {
            if (_min < val) {
                _min = val;
                JoinRuntimeFilter::_update_version();
            }
        } else {
            if (_max > val) {
                _max = val;
    ...
    void update_to_all_null() {
        DCHECK(JoinRuntimeFilter::_has_null);
    ...
    void update_to_empty_and_not_null() {
        if (!is_empty_range() || JoinRuntimeFilter::_has_null) {
            _init_min_max();
            JoinRuntimeFilter::_has_null = false;
            JoinRuntimeFilter::_update_version();
        }
    }

    void init(size_t hash_table_size) override {
        JoinRuntimeFilter::_size = hash_table_size;
        JoinRuntimeFilter::_bf.init(JoinRuntimeFilter::_size);
    }
    ...
    void insert(const CppType& value) {
        if (LIKELY(JoinRuntimeFilter::_bf.can_use())) {
            size_t hash = compute_hash(value);
            JoinRuntimeFilter::_bf.insert_hash(hash);
        }
    ...
    void evaluate(Column* input_column, JoinRuntimeFilter::RunningContext* ctx) const override {
        if (!JoinRuntimeFilter::_hash_partition_bf.empty()) {
            return JoinRuntimeFilter::_hash_partition_bf[0].can_use() ? _t_evaluate<true, true>(input_column, ctx)
                                                                      : _t_evaluate<true, false>(input_column, ctx);
        } else {
            return JoinRuntimeFilter::_bf.can_use() ? _t_evaluate<false, true>(input_column, ctx)
    ...
    std::string debug_string() const override {
        ...
        std::stringstream ss;
        ss << "RuntimeBF(type = " << ltype << ", bfsize = " << JoinRuntimeFilter::_size
           << ", has_null = " << JoinRuntimeFilter::_has_null;
    ...
    void compute_partition_index(const RuntimeFilterLayout& layout, const std::vector<Column*>& columns,
                                 JoinRuntimeFilter::RunningContext* ctx) const override {
        if (columns.empty() || JoinRuntimeFilter::_join_mode == TRuntimeFilterBuildJoinMode::NONE) return;
        ...
        // compute hash_values
        auto use_reduce = !ctx->compatibility &&
                          (JoinRuntimeFilter::_join_mode == TRuntimeFilterBuildJoinMode::PARTITIONED ||
                           JoinRuntimeFilter::_join_mode == TRuntimeFilterBuildJoinMode::SHUFFLE_HASH_BUCKET);
        if (use_reduce) {
            dispatch_layout<WithModuloArg<ReduceOp>::HashValueCompute>(
                    JoinRuntimeFilter::_global, layout, columns, num_rows, JoinRuntimeFilter::_hash_partition_bf.size(),
                    _hash_values);
        } else {
...
private:
    ...
    bool _test_data(CppType value) const {
        DCHECK(JoinRuntimeFilter::_bf.can_use());
        size_t hash = compute_hash(value);
        return JoinRuntimeFilter::_bf.test_hash(hash);
    }
    ...
    bool _test_data_with_hash(CppType value, const uint32_t shuffle_hash) const {
        ...
        DCHECK(JoinRuntimeFilter::_hash_partition_bf[bucket_idx].can_use());
        size_t hash = compute_hash(value);
        return JoinRuntimeFilter::_hash_partition_bf[bucket_idx].test_hash(hash);
    }
    ...
    void _t_evaluate(Column* input_column, JoinRuntimeFilter::RunningContext* ctx) const {
        ...
        if (input_column->is_constant()) {
            ...
            if (const_column->only_null()) {
                _selection[0] = JoinRuntimeFilter::_has_null;
            } else {
            ...
            if (nullable_column->has_null()) {
                ...
                for (int i = 0; i < size; i++) {
                    if (null_data[i]) {
                        _selection[i] = JoinRuntimeFilter::_has_null;
                    } else {
                        if constexpr (can_use_bf) {
