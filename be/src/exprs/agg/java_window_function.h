...
public:
    ...
    void update_batch_single_state_with_frame(FunctionContext* ctx, AggDataPtr __restrict state, const Column** columns,
        ...
        int num_rows = (int)columns[0]->size();
        ...
        ctx->udaf_ctxs()->_func->window_update_batch(data(state).handle, (int)peer_group_start, (int)peer_group_end, (int)frame_start,
                                                     (int)frame_end, num_args, args.data());
        // release input cols
    ...
    void get_values(FunctionContext* ctx, ConstAggDataPtr __restrict state, Column* dst, size_t start,
        ...
        for (int i = 0; i < sz; ++i) {
            assign_jvalue(desc, dst, (int)(start + i), val);
        }
