...
struct UDFFunctionCallHelper {
    ...
    ColumnPtr call(FunctionContext* ctx, Columns& columns, size_t size) {
        ...
        auto st = JavaDataTypeConverter::convert_to_boxed_array(ctx, &buffers, input_cols.data(), num_cols, (int)size,
                                                                &input_col_objs);
        RETURN_IF_UNLIKELY(!st.ok(), ColumnHelper::create_const_null_column(size));

        // call UDF method
        jobject res = helper.batch_call(fn_desc->call_stub.get(), input_col_objs.data(), (int)input_col_objs.size(), (int)size);
    ...
    ColumnPtr get_boxed_result(FunctionContext* ctx, jobject result, size_t num_rows) {
        ...
        helper.get_result_from_boxed_array(ctx, type_desc.type, res.get(), result, (int)num_rows);
