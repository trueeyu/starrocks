...
Status OlapTableSinkOperator::push_chunk(RuntimeState* state, const ChunkPtr& chunk) {
    LOG(ERROR) << "LXH: OlapTableSinkOperator::push_chunk: " << state->db() << ":" << _is_open_done;
    ...
    if (config::lxh_mode == 1) {
        LOG(ERROR) << "LXH_CORE: LocalTabletsChannel::push_chunk 1: " << (_automatic_partition_chunk != nullptr);
        return Status::InternalError("OlapTableSinkOperator::push_chunk");
    } else if (config::lxh_mode == 2) {
        sleep(2);
        LOG(ERROR) << "LXH_CORE: LocalTabletsChannel::push_chunk 2: " << (_automatic_partition_chunk != nullptr);
    } else if (config::lxh_mode == 3) {
        sleep(3);
        LOG(ERROR) << "LXH_CORE: LocalTabletsChannel::push_chunk 3: " << (_automatic_partition_chunk != nullptr);
    }
