...
bool Analytor::find_and_check_partition_end() {
    ...
    int64_t last_found_pos = _found_partition_end;
    _found_partition_end = static_cast<int64_t>(_partition_columns[0]->size());
    for (auto& column : _partition_columns) {
        _found_partition_end =
                _find_first_not_equal(column.get(), _partition_end, last_found_pos, _found_partition_end);
    }
...
void Analytor::find_peer_group_end() {
    ...
    for (auto& column : _order_columns) {
        found_peer_group_end =
                _find_first_not_equal(column.get(), _peer_group_end, _peer_group_start, found_peer_group_end);
    }
