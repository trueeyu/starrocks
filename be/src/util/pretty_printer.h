    ...
    static ENABLE_IF_ARITHMETIC(T, std::string) print(T value, TUnit::type unit, bool verbose = false) {
        ...
        case TUnit::CPU_TICKS: {
            if (value < CpuInfo::cycles_per_ms()) {
                ss << std::setprecision(PRECISION) << ((double)value / 1000.) << "K clock cycles";
            } else {
...
private:
    ...
    static double get_byte_unit(T value, std::string* unit) {
        if (value == 0) {
            ...
            *unit = "GB";
            return (double)value / (double)GIGABYTE;
        } else if (value >= MEGABYTE) {
            *unit = "MB";
            return (double)value / (double)MEGABYTE;
        } else if (value >= KILOBYTE) {
            *unit = "KB";
            return (double)value / (double)KILOBYTE;
        } else {
            *unit = "B";
            return (double)value;
        }
    ...
    static double get_unit(T value, std::string* unit) {
        if (value >= BILLION) {
            *unit = "B";
            return (double)value / (1. * BILLION);
        } else if (value >= MILLION) {
            *unit = "M";
            return (double)value / (1. * MILLION);
        } else if (value >= THOUSAND) {
            *unit = "K";
            return (double)value / (1. * THOUSAND);
        } else {
            *unit = "";
            return (double)value;
        }
