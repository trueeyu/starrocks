...
BetaRowsetWriter::BetaRowsetWriter(const RowsetWriterContext& context) : _context(context) {}
...
Status BetaRowsetWriter::init() {
    ...
    _rowset_meta_pb = std::make_shared<RowsetMetaPB>();
    ...
    _rowset_meta_pb->set_tablet_schema_hash(static_cast<int32_t>(_context.tablet_schema_hash));
...
StatusOr<RowsetSharedPtr> BetaRowsetWriter::build() {
    ...
    rowset_meta->init_from_pb(*_rowset_meta_pb);
