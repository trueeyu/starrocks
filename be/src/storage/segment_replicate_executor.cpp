...
Status ReplicateChannel::async_segment(SegmentPB* segment, butil::IOBuf& data, bool eos,
    ...
    LOG(ERROR) << "LXH_CORE: asnyc_segment start: _wait_response: ";
    Status tmp_st = _wait_response(replicate_tablet_infos, failed_tablet_infos);
    LOG(ERROR) << "LXH_CORE: asnyc_segment end: _wait_response: " << tmp_st.to_string();
    ...
    if (eos || _mem_tracker->any_limit_exceeded()) {
        LOG(ERROR) << "LXH_CORE: asnyc_segment start: eos_wait_response: ";
        auto tmp_st2 = _wait_response(replicate_tablet_infos, failed_tablet_infos);
        LOG(ERROR) << "LXH_CORE: asnyc_segment end: eos_wait_response: " << tmp_st2.to_string();
        RETURN_IF_ERROR(tmp_st2);
    }
...
void ReplicateChannel::_send_request(SegmentPB* segment, butil::IOBuf& data, bool eos, int i) {
    ...
    if (i == 0) {
        ...
        _closure->cntl.set_timeout_ms(20000);
    }
    ...
    LOG(ERROR) << "LXH: LocalTabletsChannel: send segment: " << request.id();
...
void ReplicateToken::_sync_segment(std::unique_ptr<SegmentPB> segment, bool eos) {
    ...
    for (const auto& [_, channel] : _replicate_channels) {
        LOG(ERROR) << "LXH_CORE: SegmentReplicateTask start: _sync_segment channel: " << channel->debug_string();
        auto st = Status::OK();
