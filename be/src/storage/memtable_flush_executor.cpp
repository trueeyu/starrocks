...
Status FlushToken::submit(std::unique_ptr<MemTable> memtable, bool eos,
    ...
    if (memtable != nullptr) {
        ...
        LOG(ERROR) << "LXH: BEFORE FLUSH MEMTABLE: " << eos;
    }
    Status st = _flush_token->submit(std::move(task));
    LOG(ERROR) << "LXH: AFTER FLUSH MEMTABLE: "<< eos;
    return st;
}
...
Status MemTableFlushExecutor::init(const std::vector<DataDir*>& data_dirs) {
    ...
    Status st = ThreadPoolBuilder("memtable_flush") // mem table flush
            .set_min_threads(min_threads)
            .set_max_threads(max_threads)
            .build(&_flush_pool);
    _flush_pool->set_thread_pool_name("memtable_flush");
    return st;
}
