...
public:
    ...
    // Thrift representation of the partition-key value expressions
    // (constant/literal, e.g. hdfs://path/x=1/y=2/zzz → partition slots [x, y]
    // and partition key values [1, 2]).
    //
    // Opened ExprContexts are intentionally NOT stored here: the descriptor is
    // owned by a query-level ObjectPool (shared across fragment instances), while
    // ExprContext::prepare captures a fragment-level RuntimeState. Keeping only
    // thrift here avoids that lifetime mismatch — each fragment builds and opens
    // its own ExprContexts from a fragment-local pool (see HiveDataSource).
