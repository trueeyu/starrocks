...
void BinaryColumnBase<T>::append(const Column& src, size_t offset, size_t count) {
    ...
    if (src.is_binary()) {
        ...
        const auto& b = down_cast<const LargeBinaryColumn&>(src);
        const unsigned char* p = &b._bytes[b._offsets[offset]];
        const unsigned char* e = &b._bytes[b._offsets[offset + count]];
    }
...
void BinaryColumnBase<T>::append_selective(const Column& src, const uint32_t* indexes, uint32_t from, uint32_t size) {
    if (src.is_large_binary()) {
        _append_selective<LargeBinaryColumn>(src, indexes, from, size);
    } else {
        _append_selective<BinaryColumn>(src, indexes, from, size);
    }
    _slices_cache = false;
}
...
void BinaryColumnBase<T>::_append_selective(const ColumnType& src, const uint32_t* indexes, uint32_t from, uint32_t size) {
    const auto& src_column = down_cast<const ColumnType&>(src);
    ...
    for (size_t i = 0; i < size; i++) {
        size_t row_idx = indexes[from + i];
        size_t str_size = src_offsets[row_idx + 1] - src_offsets[row_idx];
...
void BinaryColumnBase<T>::append_value_multiple_times(const Column& src, uint32_t index, uint32_t size) {
    if (src.is_large_binary()) {
        _append_value_multiple_times<LargeBinaryColumn>(src, index, size);
    } else {
        _append_value_multiple_times<BinaryColumn>(src, index, size);
    }
    _slices_cache = false;
}
...
template <typename T>
template <typename ColumnType>
void BinaryColumnBase<T>::_append_value_multiple_times(const ColumnType& src, uint32_t index, uint32_t size) {
    auto& src_column = down_cast<const ColumnType&>(src);
...
template <size_t copy_length, typename T>
void append_fixed_length(const Buffer<Slice>& strs, Bytes* bytes, BinaryColumnBase<T>::Offsets* offsets)
...
bool BinaryColumnBase<T>::append_strings_overflow(const Buffer<Slice>& strs, size_t max_length) {
    if (max_length <= 16) {
        append_fixed_length<16, T>(strs, &_bytes, &_offsets);
    } else if (max_length <= 32) {
        append_fixed_length<32, T>(strs, &_bytes, &_offsets);
    } else if (max_length <= 64) {
        append_fixed_length<64, T>(strs, &_bytes, &_offsets);
    } else if (max_length <= 128) {
        append_fixed_length<128, T>(strs, &_bytes, &_offsets);
    } else {
        for (const auto& s : strs) {
            const uint8_t* const p = reinterpret_cast<const Bytes::value_type*>(s.data);
            _bytes.insert(_bytes.end(), p, p + s.size);
...
Status BinaryColumnBase<T>::update_rows(const Column& src, const uint32_t* indexes) {
    ...
    if (!need_resize) {
        ...
        size_t remain_count = _offsets.size() - idx_begin - 1;
        if (remain_count > 0) {
...
size_t BinaryColumnBase<T>::filter_range(const Column::Filter& filter, size_t from, size_t to) {
    ...
    while (start_offset + batch_nums < to) {
        ...
        if (mask == 0) {
            ...
            // copy data
            size_t size = _offsets[start_offset + batch_nums] - _offsets[start_offset];
            memmove(data + _offsets[result_offset], data + _offsets[start_offset], size);

            // set offsets, try vectorized
            Offset* offset_data = _offsets.data();
            ...
            while (i < batch_nums) {
                ...
                size_t size = _offsets[start_offset + i + 1] - _offsets[start_offset + i];
                // copy date
    ...
    for (auto i = start_offset; i < to; ++i) {
        if (filter[i]) {
            ...
            size_t size = _offsets[i + 1] - _offsets[i];
            // copy data
