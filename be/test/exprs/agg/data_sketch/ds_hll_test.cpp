...
// A sketch built purely via update() (single-stage aggregation, no merge) stays in-order,
// so estimate_cardinality() keeps using the more accurate HIP estimator. Verify the
// estimate is accurate against the known true cardinality.
TEST(DataSketchesHllTest, PureUpdateInOrderEstimateIsAccurate) {
    const uint8_t log_k = 17;
    const auto tgt = datasketches::HLL_6;
    ...
    int64_t est = hll.estimate_cardinality();
    // RSE at lg_k=17 is well under 1%; allow a generous 3% tolerance.
    double err = std::abs(static_cast<double>(est - n)) / static_cast<double>(n);
    EXPECT_LT(err, 0.03) << "pure-update (in-order) estimate=" << est << " true=" << n;
}
